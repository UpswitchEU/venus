...
export const AI_ACTION_TOOL_NAME_TO_RESULT_TYPE = {
	...
	propose_advisor_copilot_draft: 'advisor_copilot_draft',
...
type AiStreamTurnRecoveryHeaderBag =
	| Record<string, string | string[] | undefined>
	| null
	| undefined;

export function readAiStreamTurnRecoveryHeader(
	headers: AiStreamTurnRecoveryHeaderBag
): boolean {
	if (!headers) return false;
	const raw =
		headers[AI_STREAM_TURN_RECOVERY_HEADER] ??
...
export function withAiStreamTurnRecoveryHeader(
...
export function aiRequestRecord(
	data: AiToolResultRecord
): AiToolResultRecord | null {
	return aiRecordValue(data.request);
}

export function aiCardRecord(
	data: AiToolResultRecord
): AiToolResultRecord | null {
	return aiRecordValue(data.card);
}

export function aiPendingRequestRecord(
	data: AiToolResultRecord
): AiToolResultRecord | null {
