 ...
 * ------------
 *
 * Apollo.io-style single-finding nudge. Floats above the StudioCoPilot
 * FAB at bottom-right and shows ONE issue at a time — the highest
 * priority unresolved finding from `useStudioIssues`. Designed to be
 * suggestive, not invasive:
 *
 *   - Auto-peeks once per session, after a short settle delay so the
 *     wizard renders first.
 *   - "Later" (×) closes the peek for the rest of the session — the
 *     FAB still carries the count badge as a quiet re-entry point.
 *   - Carousel arrows let the user flip through additional findings
 *     without opening the full panel.
 *   - "Ask co-pilot" hands off to the panel; the peek collapses
 *     because the panel already shows everything.
 *
 * The peek owns its own carousel index but no snooze state — that
 * lives in the parent (`StudioCoPilot`) so it can be persisted in
 * sessionStorage and shared with the open/closed state of the panel.
 */
...
interface FindingPeekProps {
  issues: StudioIssue[]
  locale?: 'en' | 'nl'
  /** Controlled open state — parent handles auto-open + snooze. */
  open: boolean
  /** Tap "Ask co-pilot" → opens panel + sends prompt. */
  onAskAi: (issue: StudioIssue) => void
  /** Snooze the peek for this session. */
  onSnooze: () => void
}
...
export function FindingPeek({
  ...
  // Clamp the carousel cursor when items resolve and the list shrinks.
  useEffect(() => {
  ...
  // Click-outside + Escape → snooze the peek for the rest of the session.
  // Apollo-style: any explicit interaction outside the card means the
  // user has chosen not to engage right now; respect it. The card
  // re-opens only via the FAB → panel re-entry path.
  // Listeners are only mounted while the peek is open, so they're free
  // when the peek is hidden.
  useEffect(() => {
    if (!open) return
    ...
    <AnimatePresence>
      {open && issue && (
        <motion.aside
          ...
          role="complementary"
          aria-label={
            locale === 'nl' ? 'Co-pilot suggestie' : 'Co-pilot suggestion'
          }
          className={cn(
        ...
        >
          <div className="flex items-center justify-between gap-2 px-3.5 pt-2.5">
            <div className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wide">
              <span
                ...
                className={cn(
                  ...
                  SEVERITY_VISUAL[issue.severity].dot
                )}
              />
              <span className={SEVERITY_VISUAL[issue.severity].accent}>
                {SEVERITY_VISUAL[issue.severity].label[locale]}
              </span>
              {total > 1 && (
            ...
            <button
              ...
              onClick={onSnooze}
              aria-label={
                locale === 'nl' ? 'Later — sluit suggestie' : 'Later — dismiss suggestion'
              }
              title={locale === 'nl' ? 'Later' : 'Later'}
          ...
          <div className="flex items-center justify-between gap-2 border-t border-foreground/[0.06] px-2.5 py-1.5">
            ...
            >
              ...
              {locale === 'nl' ? 'Vraag co-pilot' : 'Ask co-pilot'}
            </button>
            {total > 1 && (
