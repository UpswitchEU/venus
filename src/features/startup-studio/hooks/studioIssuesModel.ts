...
import { formatEur, type LiveValuation } from './useLiveValuation'
...
export function pickStudioIssues(
...
export interface DeriveStudioIssuesParams {
  benchmark: StartupBenchmarkRow
  companyName: string
  state: StartupValuationState
  valuation: LiveValuation
...
export function deriveStudioIssuesResult({
  benchmark,
  companyName,
  state,
  valuation,
}: DeriveStudioIssuesParams): StudioIssuesResult {
  const issues = pickStudioIssues(state, valuation, benchmark, companyName)
  return {
    issues,
