...
interface OctopusImportModalProps {
  open: boolean
...
export function OctopusImportModal({
  open,
  onOpenChange,
  locale,
  isLoadingCompanies,
...
}: OctopusImportModalProps) {
  const t = useTranslations('manualInput.octopus')
      ...
      <ModalContent className="max-w-xl" variant="glass" size="lg">
        ...
        <div className="space-y-4">
          <p className="text-sm text-foreground/70">
            {t('connectHint')}{' '}
            <a
              href={buildMercuryIntegrationsUrl(locale, { accountingProvider: 'octopus' })}
