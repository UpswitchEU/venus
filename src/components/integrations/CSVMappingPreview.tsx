...
const detectCategory = (code: string, description: string): { category: MappedAccount['category']; confidence: MappedAccount['confidence'] } => {
...
const mapCSVToAccounts = (data: ParsedCSVData): MappedAccount[] => {
  const codeIndex = data.headers.findIndex(h => 
    /rekening|code|nummer/i.test(h)
  );
  const descIndex = data.headers.findIndex(h => 
    /omschrijving|naam|description/i.test(h)
  );
...
export function CSVMappingPreview({
  ...
  const revenueTotal = mappedAccounts
    ...
    .reduce((sum, a) => sum + Object.values(a.values).reduce((s, v) => s + v, 0) / Object.keys(a.values).length, 0);
  ...
  const toggleCategory = (id: string) => {
    const categories: MappedAccount['category'][] = ['revenue', 'expense', 'asset', 'liability', 'equity'];
    setMappedAccounts(prev => prev.map(a => {
      if (a.id !== id) return a;
                ...
                {filteredAccounts.map((account) => (
                  ...
                  >
                    <td className="px-4 md:px-6 py-3">
                      <Mono size="sm" className="text-foreground/70">{account.code}</Mono>
                    </td>
                    <td className="px-4 md:px-6 py-3">
                      <Body size="sm" className="text-foreground/70 truncate max-w-[120px] md:max-w-xs">
                        {account.description}
                      </Body>
                    </td>
                    <td className="px-4 md:px-6 py-3">
