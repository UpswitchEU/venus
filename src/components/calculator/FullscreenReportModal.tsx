...
import type { RightPanelView } from './CalculatorNav.types'
...
export interface FullscreenReportModalProps {
  ...
  isExporting?: boolean
  /**
   * Current right-panel view. On mobile this modal IS the report surface (the
   * inline workspace isn't rendered), so it must mirror the toolbar's view: when
   * `'graph'`, render {@link graphSlot} instead of the HTML report.
   */
  rightPanelView?: RightPanelView
  /**
   * The valuation-curve content, injected by the feature layer (keeps this
   * component from importing `features/*`). Rendered when `rightPanelView==='graph'`.
   */
  graphSlot?: ReactNode
}
...
export function FullscreenReportModal({
  ...
  const isGraph = rightPanelView === 'graph' && graphSlot != null
    ...
    <Modal open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <header className="no-print absolute top-4 right-4 z-10 flex items-center gap-1 p-1.5 rounded-xl bg-background/80 backdrop-blur-md border border-foreground/10 shadow-lg">
          {/* Zoom + print are report-specific — hidden when showing the curve. */}
          {!isGraph && (
            <>
        ...
        {/* Graph view — the valuation curve fills the modal (no zoom transform). */}
        {isGraph ? (
          <div className="flex-1 min-h-0 overflow-hidden bg-background pt-12">{graphSlot}</div>
        ) : (
