 ...
 */
export function isAccountantBillingUpgradePath(path?: CreditUpgradePath | string | null): boolean {
  return path === ACCOUNTANT_CREDIT_UPGRADE_PATH || path === 'accountant_pro'
}

/** Direct / seller flow: insufficient credits → client should upgrade to Premium */
export function isClientPremiumUpgradePath(path?: CreditUpgradePath | string | null): boolean {
