  ...
  (
    ...
    const switchElement = (
      <button
        ...
        className={cn(switchTrackVariants({ size, checked: isChecked, variant }), className)}
        {...props}
      >
        <span
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col">
          ...
          {description && <span className="text-foreground/60 text-sm">{description}</span>}
        </div>
      </label>
    )
  }
...
export const SwitchGroup = React.forwardRef<HTMLDivElement, SwitchGroupProps>(
  ({ label, description, children, className, orientation = 'vertical' }, ref) => {
    return (
      <div ref={ref} className={cn('space-y-3', className)} role="group" aria-label={label}>
        {(label || description) && (
