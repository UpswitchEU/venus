...
describe('HTMLProcessor', () => {
  it.each([
    'Starting proposal based on available figures',
    'Startvoorstel op basis van beschikbare cijfers',
    'Proposition initiale basée sur les chiffres disponibles',
  ])('removes only the legacy proposal banner: %s', (copy) => {
    const fragment = HTMLProcessor.sanitizeToFragment(
      `<section><div class="report-assurance-banner report-assurance-banner--start-proposal"><h3>${copy}</h3><p>Review figures</p></div><p data-certification="draft">Certification metadata</p><aside>Methodology limitations</aside></section>`,
      document
    )
