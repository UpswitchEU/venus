...
describe('mergeImportedLedgerAnalysisIntoBusinessContext', () => {
  it('creates business_context with analysis and provenance from batch only', () => {
    ...
    expect(
      (out._imported_ledger_analysis as { dcf_defaults?: { suggested_capex?: number } })
        .dcf_defaults?.suggested_capex
    ).toBe(12000)
  ...
  it('merges dcf_defaults over existing analysis without dropping sde_flags', () => {
    ...
    const a = out._imported_ledger_analysis as {
      dcf_defaults?: { suggested_capex?: number; average_depreciation?: number }
      sde_flags?: unknown[]
    }
