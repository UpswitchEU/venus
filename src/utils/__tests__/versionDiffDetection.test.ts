...
describe('versionDiffDetection', () => {
  ...
  describe('detectVersionChanges', () => {
    ...
    it('uses top-level revenue/EBITDA when present (matches buildValuationRequest)', () => {
      const oldSlice = {
        ...baseData,
        revenue: 3_000_000,
        ebitda: 600_000,
      ...
      const changes = detectVersionChanges(oldSlice, newSlice)
      expect(changes.revenue?.from).toBe(3_000_000)
      expect(changes.revenue?.to).toBe(3_500_000)
      expect(changes.ebitda?.from).toBe(600_000)
      expect(changes.ebitda?.to).toBe(650_000)
    })
