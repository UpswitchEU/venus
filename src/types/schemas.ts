...
export const FinancialDataSchema = z
  .object({
    ...
    lease_liabilities: z.number().min(0).optional(),
...
export const ValuationRequestSchema = z
  .object({
    ...
    restricted_cash: z.number().min(0).nullable().optional(),
    lease_liabilities: z.number().min(0).nullable().optional(),
    debt_like_items: z.number().min(0).nullable().optional(),
    normalized_nwc_target: z.number().min(0).nullable().optional(),
    wacc_evidence_contract: z
      .object({
        schema_version: z.literal('wacc_evidence.v1'),
        mode: z.literal('direct_sector_wacc'),
        value_pct: z.number().positive().max(100),
        range_low_pct: z.number().positive().max(100),
        range_high_pct: z.number().positive().max(100),
        period: z.string().regex(/^\d{4}-(0[1-9]|1[0-2])$/),
        source_type: z.string().min(3),
        source_url: z.string().url().startsWith('https://'),
        content_sha256: z.string().regex(/^[0-9a-f]{64}$/i),
        chart_locator: z.string().min(3),
        methodology: z.string().min(10),
        reviewer: z.string().min(3),
        reviewed_at: z.string().datetime({ offset: true }),
        includes_size_premium: z.literal(true),
      })
      .optional(),
