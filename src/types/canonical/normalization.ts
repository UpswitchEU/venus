 ...
 *
 * The contract is vendored (inlined) rather than re-exported from
 * @upswitch/types because each app deploys from its own standalone repo. The
 * monorepo packages/ dir is unreachable in those build contexts (Vercel clone
 * for mercury/venus, Railway Docker context for titan-api), so a re-export or a
 * file: dependency on ../../packages/types would break the build.
 */
